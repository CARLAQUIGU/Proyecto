...
$this->Write(100,'Reporte de Procesos Diciplinarios');
//salto de linea
$this->Ln(60);
}
//pie de pagina
...
// Recuperando el distrito del Usuario
$distrit=$_SESSION['id_dis'];
if($_SESSION['nivel'] == 2){
//Director Distrital
$query1="SELECT p.id, un.nombre,dis.distrito,concat(date_format(p.fecha_inicio,'%d/%m/%Y'), ' - ', p.resolucion_inicio) as auto_ini  , p.falta , 
concat(date_format(p.fecha_final,'%d/%m/%Y'), ' - ', p.resolucion_final) as auto_fin,
concat(p.sancion,' - ',date_format(p.fecha_sancion,'%d/%m/%Y'),' - ', p.resolucion_sancion)as sancion , 
'No se aperturo Denuncia Penal' as penal, un.id_distrito
from proceso p
